...
import type { ProjectDto, CreateProjectDto, UpdateProjectDto, ProjectCategoryDto, LanguageDto } from '../../../ApiServices/types/TekstilTypes';
import tekstilProjectService from '../../../ApiServices/services/TekstilProjectService';
...
const ProjectManagement: React.FC = () => {
  ...
  const loadCategories = async () => {
    try {
      const result = await tekstilProjectCategoryService.getAllProjectCategories();
      if (result.success) {
  ...
  const columns: Column<ProjectDto>[] = [
    ...
    },
    {
      ...
      header: 'Kategori',
      render: (value, row) => {
        const category = row.projectCategory || categories.find(c => c.id === row.projectCategoryId);
  ...
  const handleOpenModal = (project?: ProjectDto) => {
    if (project) {
      ...
      setFormData({
        projectCategoryId: categories[0]?.id || 0,
        slug: '',
        clientName: '',
        imageUrl: '',
        imageAlt: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProject && editingProject.id) {
        const result = await tekstilProjectService.updateProject(editingProject.id, formData as UpdateProjectDto);
        if (result.success) {
        ...
        const result = await tekstilProjectService.createProject(formData);
        if (result.success) {
      ...
      toast.error(error.message || 'Proje kaydedilirken hata oluştu');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const result = await tekstilProjectService.deleteProject(id);
      if (result.success) {
  ...
  const detailModal = (row: ProjectDto, onClose: () => void) => (
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Projeler Yönetimi</h1>
        <button
      ...
      <AdvancedDataTable
        key={refreshKey}
        endpoint={`${API_BASE_URL}/api/v1.0/tekstil/projects`}
        columns={columns}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, projectCategoryId: parseInt(e.target.value) })}
