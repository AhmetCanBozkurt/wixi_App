...
const CorporateFooter = () => {
  ...
  const footerLinks = [
    { name: t('footer.about') || 'About', href: '/about' },
    { name: t('footer.services') || 'Services', href: '/services' },
    { name: t('footer.portfolio') || 'Portfolio', href: '/references' },
    { name: t('footer.blog') || 'Blog', href: '/blog' },
    { name: t('footer.contact') || 'Contact', href: '/contact' },
  ];

  return (
    <footer className="bg-darkmode dark:bg-darkmode bg-gray-900 relative z-1 border-t border-dark_border dark:border-dark_border border-gray-800 px-6">
      <div className="container mx-auto max-w-6xl px-4">
        <div className="grid md:grid-cols-12 grid-cols-1 sm:grid-cols-12">
          {/* Left Column - Logo & CTA */}
          <div className="md:col-span-4 sm:col-span-6 col-span-12 sm:border-r border-b border-solid border-dark_border dark:border-dark_border border-gray-800 flex items-center sm:border-b-0 sm:min-h-25 py-10 shrink-0">
            <div className="sm:content-normal sm:text-start text-center content-center sm:w-auto w-full">
              <Link to="/" className="md:block flex justify-center mb-6">
          ...
          <div className="md:col-span-4 sm:col-span-6 col-span-12 sm:flex items-center sm:min-h-25 py-10 justify-center shrink-0 md:border-r border-b sm:border-b-0 border-solid border-dark_border dark:border-dark_border border-gray-800">
            <div className="flex flex-col md:items-start items-center">
              <span className="text-lg font-bold text-white pb-4 inline-block">
                {t('footer.support') || 'Support'}
              </span>
              <div className="pb-5 sm:block flex">
                <p className="text-base font-bold text-white">{t('footer.phone') || 'Phone'}</p>
                <a
              ...
              <div className="sm:block flex items-center gap-3">
                <p className="text-base font-bold text-white">{t('footer.email') || 'Email'}</p>
                <a
                  href="mailto:info@wixisoftware.com"
                  className="text-2xl text-white/50 hover:text-white"
                >
          ...
          <div className="md:col-span-4 col-span-12 border-t md:border-none border-solid border-dark_border dark:border-dark_border border-gray-800 sm:flex items-center justify-end md:min-h-25 py-10 shrink-0">
            <div className="md:w-3/4 w-full sm:text-start text-center">
              <span className="font-bold text-white pb-4 inline-block text-2xl">
                {t('footer.newsletter') || 'Subscribe newsletter'}
              </span>
              <p className="text-MistyBlue text-base pb-7 text-white/50">
                {t('footer.newsletter.desc') || 'To be updated with all the latest trends and product'}
              </p>
              <form className="newsletter-form flex rounded-lg sm:w-full w-3/4 sm:mx-0 mx-auto">
                <input
                  type="email"
                  placeholder={t('footer.newsletter.email') || 'Email*'}
      ...
      <div className="text-center gap-4 md:gap-0 flex-wrap p-7 border-t border-solid border-dark_border dark:border-dark_border border-gray-800">
        <div>
          <ul className="flex justify-center mb-4 items-center sm:gap-7 gap-3">
            {footerLinks.map((link) => (
              <li key={link.href} className="text-base text-white/50">
        ...
        <div>
          <p className="text-base text-white/50">
            © {new Date().getFullYear()} {t('footer.rights') || 'All rights reserved. Made by Wixi Software'}
          </p>
        </div>
      </div>
    </footer>
