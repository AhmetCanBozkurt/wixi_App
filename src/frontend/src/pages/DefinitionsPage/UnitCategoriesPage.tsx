...
interface UnitCategoryItem extends Record<string, unknown> {
  id: string;
  code: string;
  name: string;
  nameEn: string;
...
export const UnitCategoriesPage = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [editingItem, setEditingItem] = useState<UnitCategoryItem | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  const [refreshKey, setRefreshKey] = useState(0);
  const [formData, setFormData] = useState({ ...initialForm });

  const openModal = (item?: UnitCategoryItem) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        code: item.code,
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await apiClient.put('ref/unit-categories', { id: editingItem.id, ...formData });
        toast.success('Kayıt güncellendi');
      } else {
        await apiClient.post('ref/unit-categories', formData);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await apiClient.delete(`ref/unit-categories/${deleteId}`);
    ...
    <div className={s.container}>
      <div className={s.header}>
        <div className={s.titleArea}>
          <FaLayerGroup className={s.mainIcon} />
          <div>
            <h1 className={s.pageTitle}>Birim Kategorileri</h1>
            <p className={s.pageSubtitle}>Ağırlık, uzunluk, hacim gibi birim kategorilerini yönetin.</p>
          </div>
        </div>
      ...
      <div className={s.content}>
        <AdvancedDataTable<UnitCategoryItem>
          key={refreshKey}
          dataSource="ref/unit-categories"
          columns={[
          ...
          exportTitle="Birim_Kategorileri"
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingItem ? 'Birim Kategorisi Düzenle' : 'Yeni Birim Kategorisi Ekle'}
        size="md"
        footer={
          <>
