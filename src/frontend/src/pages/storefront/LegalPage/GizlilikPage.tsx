...
export function GizlilikPage() {
  ...
  const { data: doc, isLoading } = useLegalQuery('privacy', i18n.language);

  const title = doc?.title ?? 'Gizlilik Politikası';
  const effectiveDate = doc?.effectiveDate ?? '1 Mayıs 2026';
  const version = doc?.version ?? '3.2';
      ...
      <section className={s.hero}>
        <div className="lp-container">
          ...
          <h1 className={`${s.h1} fade-up`} data-delay="1">
            {isLoading ? <>Gizlilik Politikası {SPINNER}</> : title}
          </h1>
