...
3. **Merkezi Loglama ve Denetim İzi (Audit Logs):** Sisteme giriş çıkış logları (Login/Logout) detaylı IP Adresi, Tarayıcı Verisi (User-Agent) ile WIXI_AUDIT_LOGS tablosuna yazılacaktır. Ayrıca tüm veritabanı kayıtlarında `CreatedDate`, `CreatedUser`, `UpdatedDate` ve `UpdatedUser` alanları standart olarak bulunacaktır. CQRS işlemleri esnasında loglama mediatR altyapısından da beslenecektir. Hatalar ise ayrı log tablolarında depolanacaktır.
