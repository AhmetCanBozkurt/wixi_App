...
public class UpdatePlatformPaymentSettingsCommandHandler
    : IRequestHandler<UpdatePlatformPaymentSettingsCommand, Unit>
{
    ...
    public UpdatePlatformPaymentSettingsCommandHandler(
    ...
    public async Task<Unit> Handle(
        UpdatePlatformPaymentSettingsCommand request, CancellationToken ct)
    {
        var setting = await _db.PlatformPaymentSettings.FirstOrDefaultAsync(ct);

        if (setting is null)
        {
            setting = new WixiPlatformPaymentSetting();
            _db.PlatformPaymentSettings.Add(setting);
        }

        // Boş veya null gelirse mevcut değeri koru
